...
双语功能会读取 Blender 真实存在的 `.mo` 翻译文件，并在 Blender 安装目录的 `datafiles/locale` 下创建独立 locale 目录和 `LC_MESSAGES/blender.mo` 文件；同时会在该目录的 `languages` 文件中添加本扩展标记的语言条目。原有 Blender locale 文件不会被覆盖。
...
- `Emergency Cleanup` 仅移除本扩展清单中记录的文件，或 Blender `languages` 文件内由本扩展 marker block 声明的 locale。它不会删除未记录的 locale，也不会删除其他插件或 Blender 自身的文件。
...
- 如需恢复本扩展已记录的双语安装，可运行 `Emergency Cleanup`，重启 Blender 后重新安装；它不会清理未记录的第三方文件。
...
- `Emergency Cleanup` removes only files recorded by this extension's manifest or locale codes declared in this extension's marker block in Blender's `languages` file. It does not remove unrecorded locales, other add-ons' files, or Blender's own files.
...
- To restore an installation tracked by this extension, run `Emergency Cleanup`, restart Blender, and install again. It does not clean untracked third-party files.
